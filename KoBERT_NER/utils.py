...
## 엑셀 파일에서 라벨 불러오기
def xlsx2label(label_path, sheetname):
    label_file = load_workbook (label_path)
    label_sheet = label_file[sheetname]
    label = ["UNK", "O"]
    for tokens in label_sheet.rows:
        temp = []
        for cell in tokens:
        ...
        if temp == ["Tag", "Mean", "Color", "Visualize"]:
            ...
            label.append(temp[0]+"-B")
            label.append(temp[0]+"-I")
            
    return label

def get_labels(args, is_predict = False):
    # try:
    #     label = [label.strip () for label in open (args, 'r', encoding='utf-8')]
    # except TypeError:
    if is_predict:
        label = [label.strip () for label in open (args, 'r', encoding='utf-8')]
    else:
        label = [label.strip () for label in open (args.label_file, 'r', encoding='utf-8')]
    return label
