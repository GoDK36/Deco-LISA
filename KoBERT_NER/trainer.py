...
from KoBERT_NER.utils import compute_metrics, get_labels, get_test_texts, show_report, MODEL_CLASSES

logger = logging.getLogger(__name__)


class Trainer(object):
    def __init__(self, args, device=str, train_dataset=None, dev_dataset=None, test_dataset=None):
        self.args = args
    ...
    def train(self):
        ...
        for _ in train_iterator:
            ...
            for step, batch in enumerate(epoch_iterator):
                self.model.train()
                # 윈도우 환경 64비트 사용(효과 있는지는 잘...)
                batch = tuple(t.to(self.device).to(torch.int64) for t in batch)  # GPU or CPU
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0:
                    # 윈도우 환경 64비트 사용(효과 있는지는 잘...)
                    torch.nn.utils.clip_grad_norm_(self.model.parameters(), self.args.max_grad_norm).to(torch.int64)
